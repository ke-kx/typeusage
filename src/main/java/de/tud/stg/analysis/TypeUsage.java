...
public abstract class TypeUsage {
    ...
    /**
     * This is the behavior found in the original code. It differs with respect to the behavior explained in the papers,
     * in so far that it only takes into account methods which are in other but missing in this, without regards to
     * additional methods in this.
     * <p>
     * As an example: this has methods 1,2 and other has methods 2,3. This variant calculates the set of added methods
     * as {3} and thus declares them almost equal even though method 1 is not present in other at all!
     * <p>
    ...
    /**
     * This is the behavior as explained in the papers. other is almost equal to this, iff M(this) contained in M(other)
     * and |M(other)| = |M(this)| + k In other words, other is almost equal to this, if it calls the same methods plus
     * up to k additional ones.
     */
