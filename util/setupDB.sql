...
CREATE VIEW missingMethodCount AS
SELECT ae.id, 
...
CREATE VIEW missingMethodPercentage AS
SELECT mmc.id, mmc.methodId, CAST(mmc.count AS FLOAT) / CAST((SELECT SUM(mmc2.count) FROM missingMethodCount mmc2 WHERE mmc2.id = mmc.id GROUP BY mmc2.id) AS FLOAT) AS percentage
    FROM missingMethodCount mmc
;

-- should show strangeness score, complete typeusage and the missing calls + their frequency
CREATE VIEW missingMethodCalls AS
SELECT s.score, s.equalCount, s.almostEqualCount, tuc.class, tuc.lineNr, tuc.context, tuc.typeName, tuc.methodCalls, m.methodName AS missingCall, mmp.percentage
    FROM strangeness s JOIN showTypeusageComplete tuc ON s.id = tuc.id
    JOIN missingMethodPercentage mmp ON tuc.id = mmp.id
